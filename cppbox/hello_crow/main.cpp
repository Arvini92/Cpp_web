...
void sendHtml(response &res, string filename) {
  send(res, filename + ".html", "text/html");
}

void sendImage(response &res, string filename) {
  send(res, "images/" + filename, "image/jpeg");
}

void sendSript(response &res, string filename) {
  send(res, "scripts/" + filename, "text/javascript");
}

void sendSript(response &res, string filename) {
  send(res, "styles/" + filename, "text/css");
}
...
int main(int argc, char* argv[]) {
  ...
  monogocxx::instance inst{};
  string mongoConnect = std::string(getenv("MONGODB_URI"));
  mongocxx::client conn{mongocxx::uri{mongoConnect}};
  auto collection = conn["cppweb"]["contracts"];
  ...
  CROW_ROUTE(app, "/ws")
    ...
    .onopen([&](crow::websocket::connection &conn, const string &reason) {
      std::lock_guard<std::mutex> _(mtx);
      users.erase(&conn);
    })
  ...
  CROW_ROUTE(app, "/api/contacts")
    ([&collection](const request &req){
      ...
      for(auto doc : docs) {
        contacts.push_back(json::load(bsconcxx::to_json(doc)));
      }
      crow:json::wvalue dto;
  ...
  CROW_ROUTE(app, "/rest_test").methods(HTTPMethod::Post, HTTPMethod::Get,
        HTTPMethod::Put)
    ([](const request &req, response &res){
      sstring method = method_name(req.method);
      res.set_header("Content-Type", "text/plain");
